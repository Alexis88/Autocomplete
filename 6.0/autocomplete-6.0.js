...
let Autocomplete = {
	...
	init: _ => {
		...
		window.addEventListener("keyup", event => Autocomplete.container && Autocomplete.keys(event), false);
		window.addEventListener("keypress", event => event.which == 13 && Autocomplete.container && event.preventDefault(), false);
